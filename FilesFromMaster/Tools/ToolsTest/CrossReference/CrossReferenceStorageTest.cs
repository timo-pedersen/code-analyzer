...
namespace Neo.ApplicationFramework.Tools.CrossReference
{
    ...
    public class CrossReferenceStorageTest
    {
        ...
        private static IFontService m_FontService;
        ...
        public void SetUp()
        {
            ...
            m_FontService = MockRepository.GenerateStub<IFontService>();
            m_FontService.Stub(x => x.GetFontFile(ArialFontName)).Return(ArialFontFileName);
            m_FontService.Stub(x => x.GetFontFile(TimesNewRomanFontName)).Return(TimesNewRomanFontFileName);
            m_FontService.Stub(x => x.GetFontFile(TahomaFontName)).Return(TahomaFontFileName);
            m_FontService.Stub(x => x.GetFontInfo(Arg<FrameworkElement>.Is.Anything)).Do((Func<FrameworkElement, IFontInfo>)(element =>
            {
                IAppearanceAdapter appearanceAdapter = AppearanceAdapterService.GetAdapter(element);
                if (appearanceAdapter == null)
                    return null;

                return appearanceAdapter.GetFontInfo();
            }));

            TestHelper.AddService<IFontService>(m_FontService);
        }
        ...
        private static void AssertFontReferenceItemEquals(ICrossReferenceItem crossReferenceItem, string elementName, string fontFileName)
        {
            ...
            Assert.IsTrue(m_FontService.GetFontFile(crossReferenceItem.SourceFullName).ToLower().Contains(fontFileName.ToLower()));
        }
        ...
        private static class ProviderHelper
        {
            ...
            private static void RegisterProvidersForScreenDesigners(ICrossReferenceServiceSignature crossReferenceService)
            {
                ...
                crossReferenceService.RegisterCrossReferenceProvider(new ScreenFontDesignerCrossReferenceProvider());
