...
namespace Neo.ApplicationFramework.Tools.Screen.ScreenEditor.Input
{
    ...
    public class ScreenEditorCommandsTest
    {
        ...
        public void SelectNextObjectWithTabKey()
        {
            ...
            m_ScreenRootDesigner.AssertWasCalled(
                x => x.Select(m_SecondaryMasterRectangle, SelectionTypes.Replace), 
                y => y.Repeat.AtLeastOnce());
            Assert.IsTrue(result);
        }
        ...
        public void Copy()
        {
            ...
            m_ScreenDesignerView.Stub(x => x.GetScreenDataObject()).Return(MockRepository.GenerateStub<IScreenDataObject>());

            bool result = m_ScreenEditorCommands.Copy();

            m_ScreenDesignerView.AssertWasCalled(
                x => x.GetScreenDataObject(), 
                y => y.Repeat.AtLeastOnce());
            Assert.IsTrue(result);
        }
        ...
        public void Cut()
        {
            //Stub.On(m_ScreenDesignerView).Method("Cut").WithNoArguments().Will(Return.Value(null));
        ...
        {
            FrameworkElement screenStub = MockRepository.GenerateStub<ScreenWindowMock>();
